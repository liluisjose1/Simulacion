...
class MyFrame1 ( wx.Frame ):
    ...
    def __init__( self, parent ):
        ...
        wx.Frame.__init__ ( self, parent, id = wx.ID_ANY, title = self.title, pos = wx.DefaultPosition, size = wx.Size( 610,350 ), style = wx.DEFAULT_FRAME_STYLE|wx.TAB_TRAVERSAL )
    ...
    def Eliminar( self, event ):
        form_hijo=MyDialog2(self)
        form_hijo.Show()
        form_hijo.Mostrar(self.item2)
...
class MyDialog1 ( wx.Dialog ):
    ...
    def __init__( self, parent ):
        wx.Dialog.__init__ ( self, parent, id = wx.ID_ANY, title = u"CRUD Productos", pos = wx.DefaultPosition, size = wx.Size( 760,480 ), style = wx.DEFAULT_DIALOG_STYLE )
        ...
        self.btn4 = wx.Button( self, wx.ID_ANY, u"Atras", wx.DefaultPosition, wx.DefaultSize, 0 )
    ...
    def Mostrar(self,accion):
        ...
        if ( self.lbl1.GetLabel()=='INSERTAR'):
            ...
            if (self.id_producto!=''):
                ...
                tiene_imagen=int(self.rows[10]) 
                if tiene_imagen == 0: #row[8] es la posicion del cursor del campo imagen
                    self.ruta='imgs/blank_img.png' 
                    data=self.ruta
                else:
            ...
            if (self.id_producto!=''):
                ...
                self.txt1.SetValue(str(self.rows[0]))
    ...
    def btnAceptar( self, event ):
        if ( self.lbl1.GetLabel()=='EDITAR'):
            ...
            sql="""UPDATE producto SET barcode=%s,id_categoria=%s,descripcion=%s,exento=%s, costo=%s,precio=%s,inactivo=%s,tiene_imagen=%s
                WHERE id_producto=%s"""
            valores=(self.barcode,self.id_cat,self.descripcion,self.exento,self.costo,self.precio,self.inactivo,'1',self.id_producto)
            typesql='U'
            ...
            #binario = pymysql.Binary(imagen)#trasnforma img para almacenar en mysql
            sql="""INSERT INTO `producto`(`barcode`,imagen,tiene_imagen,`fecha_ingreso`, `id_categoria`, `descripcion`, `exento`, `costo`, `precio`, `inactivo`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
            valores=(self.barcode,imagen,1,fecha,self.id_cat,self.descripcion,self.exento,self.costo,self.precio,self.inactivo)
            typesql='I'
            query=self.db.query(sql,valores,typesql)  
            self.dial.ShowModal()
            self.frm_padre.Cargar()
...
class MyDialog2 ( wx.Dialog ):
    ...
    def __init__( self, parent ):
        wx.Dialog.__init__ ( self, parent, id = wx.ID_ANY, title = wx.EmptyString, pos = wx.DefaultPosition, size = wx.Size( 556,98 ), style = wx.DEFAULT_DIALOG_STYLE )
        self.frm_padre=parent
        #self.SetSizeHints( wx.DefaultSize, wx.DefaultSize )
        ...
        bSizer1.Add( ( 30, 30), 1, wx.EXPAND, 5 )
        
        self.m_staticText1 = wx.StaticText( self, wx.ID_ANY, u"Seguro que deseas eliminar id", wx.DefaultPosition, wx.DefaultSize, 0 )
        self.m_staticText1.Wrap( -1 )
        ...
        self.m_button1 = wx.Button( self, wx.ID_ANY, u"Aceptar", wx.DefaultPosition, wx.DefaultSize, 0 )
        bSizer2.Add( self.m_button1, 0, wx.ALL, 5 )
        
        self.m_button2 = wx.Button( self, wx.ID_ANY, u"Cerrar", wx.DefaultPosition, wx.DefaultSize, 0 )
        ...
        self.Centre( wx.BOTH )
        # Connect Events
        self.m_button1.Bind( wx.EVT_BUTTON, self.Aceptar )
        self.m_button2.Bind( wx.EVT_BUTTON, self.Cerrar )
        #base de datos
        self.db = mysqlclass3.Database("ventas") #Instanciar la conexion a la Bd.
        #Evento mostrar datos
        
        #MODAL PARA mensajes
        self.dial = wx.MessageDialog(None, 'Eliminado con exito', 'Info', wx.OK|wx.CENTRE)
    
    def __del__( self ):
        pass
    
    def Mostrar (self, action):
        self.m_textCtrl1.SetValue(str(self.frm_padre.item2))
    ...
    def Aceptar( self, event ):
        #event.Skip()
        id_producto=str(self.m_textCtrl1.GetValue())  
